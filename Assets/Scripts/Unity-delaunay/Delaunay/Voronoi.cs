...
namespace Delaunay
{
    public sealed class Voronoi : Utils.IDisposable
    {
        ...
        public Voronoi(List<Vector2> points, List<uint> colors, Rect plotBounds)
        {
            ...
            AddSites(points, colors, out removeIndex);
        ...
        public Voronoi(List<Vector2> points, List<uint> colors, Rect plotBounds, out List<bool> removeIndex)
        {
            _sites = new SiteList();
            _sitesIndexedByLocation = new Dictionary<Vector2, Site>(); // XXX: Used to be Dictionary(true) -- weak refs. 
            AddSites(points, colors, out removeIndex);
        ...
        private void AddSites(List<Vector2> points, List<uint> colors, out List<bool> removeIndex)
        {
            removeIndex = new List<bool>();
        ...
        private bool AddSite(Vector2 p, uint color, int index)
        {
            if (_sitesIndexedByLocation.ContainsKey(p))
                return false; // Prevent duplicate site! (Adapted from https://github.com/nodename/as3delaunay/issues/1)
        ...
        private void FortunesAlgorithm()
        {
            ...
            for (; ; )
            {
                ...
                else if (heap.Empty() == false)
                {
                    ...
                    _triangles.Add(new Triangle(bottomSite, topSite, FortunesAlgorithm_rightRegion(rrbnd)));
